...
(function () {
	...
	class StateManager {
		...
		async _setStateImmediate(path, value, context = {}) {
			const oldValue = this.getState(path);

			let finalValue = value;

			// Process middleware sequentially to handle async middleware
			...
			if (!this.isUpdating) {
				...
				await this._notifySubscribers(path, finalValue, oldValue);
				await this._notifyExternalSubscribers(path, finalValue, oldValue);
		...
		async _notifySubscribers(path, newValue, oldValue) {
			await this._triggerPathSubscribers(path);

			const parts = getPathParts(path);
			for (let i = parts.length - 1; i > 0; i--) {
				const parentPath = parts.slice(0, i).join('.');
				await this._triggerPathSubscribers(parentPath);
			}
			...
			for (const subscriberPath of allSubscriberPaths) {
				if (subscriberPath.startsWith(prefix) && subscriberPath !== path) {
					await this._triggerPathSubscribers(subscriberPath);
				}
			}
		}

		async _notifyExternalSubscribers(changedPath, newValue, oldValue) {
			const notifications = [];
			...
			this.externalSubscribers.forEach((subscriptions, subscribedPath) => {
				subscriptions.forEach(subscription => {
					...
					if (shouldNotify) {
						notifications.push(() => callback(newValue, oldValue, changedPath));
					}
				});
			});

			// Execute notifications sequentially to handle async callbacks
		...
		async _triggerPathSubscribers(path) {
			const subs = this.subscribers.get(path);
			if (subs) {
	...
	class ComponentManager {
		...
		async create(name, props = {}) {
			...
			try {
				...
				// FIXED: Await component function execution
				const result = await Promise.resolve(componentFn(props, context));
				...
				if (result && typeof result === 'object') {
					...
					if (result.onMount || result.onUpdate || result.onUnmount ||
						...
						return await this._createLifecycleComponent(result, name, props, componentStates);
					}

					// Check for render function pattern
					if (typeof result.render === 'function' && !result.onMount && !result.onUpdate && !result.onUnmount) {
						const renderResult = await Promise.resolve(result.render());
						console.log(`Render function for '${name}' returned:`, renderResult);
						const element = await this.juris.domRenderer.render(renderResult);
					...
					if (keys.length === 1) {
						...
						if (typeof tagName === 'string' && tagName.length > 0) {
							const element = await this.juris.domRenderer.render(result);
							if (element && componentStates.size > 0) {
								this.componentStates.set(element, componentStates);
							}
							return element;
		...
		async _createLifecycleComponent(componentResult, name, props, componentStates) {
			const instance = {
				name,
				props,
			...
			// FIXED: Await render operation and handle async render functions
			const renderResult = await Promise.resolve(instance.render());
			const element = await this.juris.domRenderer.render(renderResult);
				...
				if (instance.hooks.onMount) {
					setTimeout(async () => {
						if (element.isConnected) {
							try {
								await Promise.resolve(instance.hooks.onMount());
							} catch (error) {
		...
		async updateInstance1(element, newProps) {
			...
			if (instance.hooks.onUpdate) {
				try {
					await Promise.resolve(instance.hooks.onUpdate(oldProps, newProps));
				} catch (error) {
			...
			try {
				const newContent = await Promise.resolve(instance.render());
				await this.juris.domRenderer.updateElementContent(element, newContent);
			} catch (error) {
		...
		async cleanup(element) {
			...
			if (instance && instance.hooks.onUnmount) {
				try {
					await Promise.resolve(instance.hooks.onUnmount());
				} catch (error) {
	...
	class DOMRenderer {
		...
		async render(vnode) {
			...
			if (Array.isArray(vnode)) {
				...
				const childElements = await Promise.all(
					vnode.map(child => this.render(child))
				);
			...
			if (this.juris.componentManager.components.has(tagName)) {
				...
				const result = await this.juris.componentManager.create(tagName, props);
			...
			if (this.renderMode === 'fine-grained') {
				return await this._createElementFineGrained(tagName, props);
			} else {
				return await this._createElementOptimized(tagName, props, props.key);
			}
		}

		// FINE-GRAINED: Direct DOM manipulation method
		async _createElementFineGrained(tagName, props) {
			...
			for (const [key, value] of Object.entries(props)) {
				if (key === 'children') {
					await this._handleChildrenFineGrained(element, value, subscriptions);
				} else if (key === 'text') {
					await this._handleText(element, value, subscriptions);
				} else if (key === 'style') {
					await this._handleStyleFineGrained(element, value, subscriptions);
				} else if (key.startsWith('on')) {
					this._handleEvent(element, key, value, eventListeners);
				} else if (typeof value === 'function') {
					await this._handleReactiveAttribute(element, key, value, subscriptions);
				} else if (key !== 'key') {
		...
		async _handleChildrenFineGrained(element, children, subscriptions) {
			if (typeof children === 'function') {
				...
				const updateChildren = async () => {
					try {
						...
						if (result !== "ignore") {
							await this._updateChildrenFineGrained(element, result);
				...
				await this._updateChildrenFineGrained(element, children);
			}
		}

		async _updateChildrenFineGrained(element, children) {
			if (children === "ignore") {
				return;
			}
			...
			if (Array.isArray(children)) {
				const childElements = await Promise.all(
					children.map(child => this.render(child))
				);
				childElements.forEach(childElement => {
					if (childElement) {
						fragment.appendChild(childElement);
				...
				const childElement = await this.render(children);
				if (childElement) {
					fragment.appendChild(childElement);
				}
			}
		...
		async _handleStyleFineGrained(element, style, subscriptions) {
			if (typeof style === 'function') {
				let lastStyleValue = null;
				let isInitialized = false;

				const updateStyle = async () => {
					const styleObj = await Promise.resolve(style());
		...
		async _createElementOptimized(tagName, props, key) {
			...
			await this._processProperties(element, props, subscriptions, eventListeners);
		...
		async _processProperties(element, props, subscriptions, eventListeners) {
			for (const [key, value] of Object.entries(props)) {
				if (key === 'children') {
					await this._handleChildrenOptimized(element, value, subscriptions);
				} else if (key === 'text') {
					await this._handleText(element, value, subscriptions);
					...
					await this._handleStyle(element, value, subscriptions);
				} else if (key.startsWith('on')) {
					this._handleEvent(element, key, value, eventListeners);
				} else if (typeof value === 'function') {
					await this._handleReactiveAttribute(element, key, value, subscriptions);
		...
		async _handleChildrenOptimized(element, children, subscriptions) {
			if (typeof children === 'function') {
				...
				const updateChildren = async () => {
					try {
						const newChildren = await Promise.resolve(children());
						if (newChildren !== "ignore" && !this._childrenEqual(lastChildrenState, newChildren)) {
							if (useOptimizedPath) {
								try {
									childElements = await this._reconcileChildren(element, childElements, newChildren);
									...
									await this._updateChildrenSafe(element, newChildren);
									lastChildrenState = newChildren;
								}
							} else {
								await this._updateChildrenSafe(element, newChildren);
						...
						useOptimizedPath = false;
						try {
							await this._updateChildrenSafe(element, []);
						} catch (fallbackError) {
				...
				try {
					const initialChildren = await Promise.resolve(children());
					childElements = await this._reconcileChildren(element, [], initialChildren);
					lastChildrenState = initialChildren;
				} catch (error) {
					console.warn('Initial reconciliation failed, using safe method:', error.message);
					useOptimizedPath = false;
					const initialChildren = await Promise.resolve(children());
					await this._updateChildrenSafe(element, initialChildren);
					lastChildrenState = initialChildren;
				}

			} else {
				try {
					await this._reconcileChildren(element, [], children);
				} catch (error) {
					console.warn('Static reconciliation failed, using safe method:', error.message);
					await this._updateChildrenSafe(element, children);
				}
			}
		}

		async _updateChildrenSafe(element, children) {
			if (children === "ignore") {
				return;
			}
			...
			if (Array.isArray(children)) {
				const childElements = await Promise.all(
					children.map(async child => {
						try {
							return await this.render(child);
						} catch (error) {
		...
		async _reconcileChildren(parent, oldChildren, newChildren) {
			...
			for (let index = 0; index < newChildren.length; index++) {
				...
				if (existingElement &&
					...
					const newElement = await this.render(newChild);
		...
		async _handleText(element, text, subscriptions) {
			if (typeof text === 'function') {
				let lastTextValue = null;
				let isInitialized = false;

				const updateText = async () => {
					const newTextValue = await Promise.resolve(text());
		...
		async _handleStyle(element, style, subscriptions) {
			if (typeof style === 'function') {
				...
				const initialStyle = await Promise.resolve(style());
				...
				if (Object.keys(reactiveProps).length > 0) {
					...
					const updateReactiveStyles = async () => {
						const changes = {};
						let hasChanges = false;

						for (const prop of Object.keys(reactiveProps)) {
							const newValue = await Promise.resolve(reactiveProps[prop]());
					...
					this._createReactiveUpdate(element, updateReactiveStyles, subscriptions);

					// Initialize reactive styles
					for (const prop of Object.keys(reactiveProps)) {
						const value = await Promise.resolve(reactiveProps[prop]());
		...
		async _handleReactiveAttribute(element, attr, valueFn, subscriptions) {
			let lastValue = null;
			let isInitialized = false;

			const updateAttribute = async () => {
				const newValue = await Promise.resolve(valueFn());
		...
		_createReactiveUpdate(element, updateFn, subscriptions) {
			...
			try {
				// Initial call to capture dependencies
				updateFn();
			} catch (error) {
			...
			dependencies.forEach(path => {
				...
				const unsubscribe = this.juris.stateManager.subscribeInternal(path, wrappedUpdateFn);
				subscriptions.push(unsubscribe);
			});
		}

		async _updateChildren(element, children) {
			if (children === "ignore") {
				return;
			}
			...
			try {
				if (Array.isArray(children)) {
					...
					await this._reconcileChildren(element, currentChildElements, children);
				} else {
					const childrenArray = children ? [children] : [];
					const currentChildElements = Array.from(element.children);
					await this._reconcileChildren(element, currentChildElements, childrenArray);
				}
			} catch (error) {
		...
		// FIXED: Element content update async
		async updateElementContent(element, newContent) {
			await this._updateChildren(element, [newContent]);
		}
...
*/
	class DOMEnhancer {
		...
		async _enhanceSelectorElement(element, definition, container, selector) {
			try {
				...
				if (typeof definition === 'function') {
					...
					actualDefinition = await Promise.resolve(definition(context));
				...
				const processedDefinition = await this._processElementAwareFunctions(element, actualDefinition);

				await this._applyEnhancements(element, processedDefinition);
		...
		async _processElementAwareFunctions(element, definition) {
			const processed = {};
		...
		async _enhanceElement(element, definition, config) {
			if (this.enhancedElements.has(element)) {
				return;
			}
			...
			try {
				...
				await this._applyEnhancements(element, actualDefinition);

				if (config.onEnhanced) {
					const context = this.juris.createContext(element);
					await Promise.resolve(config.onEnhanced(element, context));
				}
		...
		async _applyEnhancements(element, definition) {
			const subscriptions = [];
			const eventListeners = [];
			...
			for (const [key, value] of Object.entries(definition)) {
				try {
					if (key === 'children') {
						await this._handleChildren(element, value, subscriptions, renderer);
					} else if (key === 'text') {
						await renderer._handleText(element, value, subscriptions);
					} else if (key === 'innerHTML') {
						await this._handleInnerHTML(element, value, subscriptions, renderer);
					} else if (key === 'style') {
						await renderer._handleStyle(element, value, subscriptions);
					} else if (key.startsWith('on')) {
						renderer._handleEvent(element, key, value, eventListeners);
					} else if (typeof value === 'function') {
						await renderer._handleReactiveAttribute(element, key, value, subscriptions);
					} else {
		...
		async _handleChildren(element, children, subscriptions, renderer) {
			if (renderer.isFineGrained()) {
				await renderer._handleChildrenFineGrained(element, children, subscriptions);
			} else {
				await renderer._handleChildrenOptimized(element, children, subscriptions);
			}
		}

		async _handleInnerHTML(element, innerHTML, subscriptions, renderer) {
			if (typeof innerHTML === 'function') {
				await renderer._handleReactiveAttribute(element, 'innerHTML', innerHTML, subscriptions);
			} else {
	...
	*/
	class Juris {
		...
		async initializeHeadlessComponent(name, props) {
			return await this.headlessManager.initialize(name, props);
		}
		...
		async registerAndInitHeadless(name, componentFn, options = {}) {
			this.headlessManager.register(name, componentFn, options);
			return await this.headlessManager.initialize(name, options);
		}
		 ...
		 */
		async objectToHtml(vnode) {
			return await this.domRenderer.render(vnode);
		}
		...
		async render(container = '#app') {
			...
			await this.headlessManager.initializeQueued();
			...
			try {
				...
				const element = await this.domRenderer.render(this.layout);
				if (element) {
  ...
  `;
		...
		// Public API - Cleanup
		async cleanup() {
			await this.headlessManager.cleanup();
		}
